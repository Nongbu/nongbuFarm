<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<%@ page import="java.util.ArrayList, exchange.model.vo.Exchange, java.sql.Date, member.model.vo.Member" %> 
<%
...
<script type="text/javascript">
	...
	function showWriteForm(){
		location.href="/nongbu/views/exchange/exchangeWriteForm.jsp?page=<%= currentPage%>";
	}
</script>
...
<div id="titleDiv">
	<form action="/nongbu/esearch" method="post">
...
<tr>
	<td align="center"><%= exchange.getExNo() %></td>
	<td>
	<%--댓글일때는 제목을 들여쓰기함 --%>
	<% if(exchange.getExReplyLev() == 2){ //원글의 댓글 일 때  %>
	&nbsp; &nbsp; ▶
	<% }else if(exchange.getExReplyLev() == 3){ //댓글의 댓글 일 때 %>
	&nbsp; &nbsp; &nbsp; &nbsp; ▶▶
	<% } %>
	<%-- 로그인 한 사용자만 게시글 상세보기 할 수 있게 처리함 --%>
	<% if( loginMember != null ){ %>
	<a href="/nongbu/edetail?bnum=<%=exchange.getExNo() %>&page=<%= currentPage%>"><%= exchange.getExTitle() %></a>
	<% }else{ %>
	<%= exchange.getExTitle() %>
	<% } %>
	</td>
	<td align="center" ><%= exchange.getExWriter() %></td>
	<td align="center" ><%= exchange.getExDate() %></td>
	<td align="center" ><%= exchange.getExReadCount() %></td>
	<td align="center" >
...
[맨처음]
<% }else{ %>
<a href="/nongbu/elist?page=1">[맨처음]</a>
<% } %> &nbsp;

<%-- 이전 그룹이 있으면 링크설정, 이전 그룹 없으면 링크 없음 --%>
<% if((currentPage -10) < startPage && (currentPage - 10) >= 1){ %>
<a href="/nongbu/elist?page=<%= startPage - 10 %>">[이전]</a>
<% }else{ %>
[이전]
<% } %> &nbsp;
...
<% }else{  %>
<a href="/nongbu/elist?page=<%= p %>"><%= p %></a>
<% }//else 닫기괄호
		} //for 닫기괄호 %> &nbsp;
		
<%-- 다음 그룹이 있으면 링크설정, 다음 그룹 없으면 링크 없음 --%>
<% if((currentPage + 10) > endPage && (currentPage + 10) < maxPage){ %>
<a href="/nongbu/elist?page=<%= endPage + 10 %>">[다음]</a>
<% }else{ %>
[다음]
<% } %> &nbsp;
...
[맨끝]
<% }else{ %>
<a href="/nongbu/elist?page=<%= maxPage %>">[맨끝]</a>
<% } %> &nbsp;
